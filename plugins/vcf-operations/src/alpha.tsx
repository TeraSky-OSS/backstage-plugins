...
const isVCFOperationsAvailable = (entity: Entity) => {
  return Boolean(
    entity.metadata.annotations?.[
      'terasky.backstage.io/vcf-automation-resource-type'
    ],
  );
};
  ...
  params: defineParams =>
    defineParams({
      ...
      factory: ({ discoveryApi, fetchApi }) =>
        new VcfOperationsClient({ discoveryApi, fetchApi }),
    }),
  disabled: false,
});
...
export const vcfOperationsContent: ExtensionDefinition =
  EntityContentBlueprint.make({
    ...
    params: {
      ...
      loader: () =>
        import('./components/VCFOperationsExplorer').then(m => (
          <m.VCFOperationsExplorer />
        )),
    },
    disabled: false,
  });
...
/** @alpha */
export const vcfOperationsPlugin: FrontendPlugin = createFrontendPlugin({
