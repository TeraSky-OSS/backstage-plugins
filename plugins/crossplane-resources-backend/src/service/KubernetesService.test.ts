...
describe('KubernetesService', () => {
  ...
  describe('getV2ResourceGraph', () => {
    ...
    it('should handle nested managed resources', async () => {
      server.use(
        rest.get('http://kubernetes-backend/proxy/apis/test.example.com/v1/namespaces/default/composites/my-composite', (req, res, ctx) => {
          return res(ctx.json({
            apiVersion: 'test.example.com/v1',
        ...
        rest.get('http://kubernetes-backend/proxy/apis/nested.example.com/v1/namespaces/default/nestedcomposites/nested-1', (req, res, ctx) => {
          return res(ctx.json({
            apiVersion: 'nested.example.com/v1',
            kind: 'NestedComposite',
            metadata: { name: 'nested-1', namespace: 'default', uid: 'nested-uid' },
            spec: {
              crossplane: {
                resourceRefs: [
        ...
        rest.get('http://kubernetes-backend/proxy/apis/leaf.example.com/v1/namespaces/default/leafresources/leaf-1', (req, res, ctx) => {
          return res(ctx.json({
            apiVersion: 'leaf.example.com/v1',
            kind: 'LeafResource',
            metadata: { name: 'leaf-1', namespace: 'default', uid: 'leaf-uid' },
          }));
        }),
      );
    ...
    it('should handle nested Object with remote manifest', async () => {
      server.use(
        ...
        rest.get('http://kubernetes-backend/proxy/apis/kubernetes.crossplane.io/v1alpha1/namespaces/default/objects/nested-obj', (req, res, ctx) => {
          return res(ctx.json({
            apiVersion: 'kubernetes.crossplane.io/v1alpha1',
            kind: 'Object',
            metadata: { name: 'nested-obj', uid: 'nested-obj-uid' },
            status: {
              atProvider: {
                manifest: {
                  apiVersion: 'v1',
                  kind: 'Secret',
                  metadata: { name: 'remote-secret' },
                },
              },
            },
          }));
